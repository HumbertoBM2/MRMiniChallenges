...
class SignalProcessor(Node):
    def __init__(self):
        ...
        self.phase_shift = np.pi / 4  # Hardcoded phase shift
    ...
    def process_signal(self, msg):
        ...
        shifted_signal = np.sin(self.time + self.phase_shift)  # Apply phase shift
        processed_signal = self.amplitude_factor * (shifted_signal + self.offset)  # Offset & scale
